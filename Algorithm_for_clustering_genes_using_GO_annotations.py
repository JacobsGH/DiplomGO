...
class ClusterLogger:
    ...
    def log_iteration(self, iteration_info):
        ...
        status = "активна" if iteration_info.get('filter_active', False) else "завершена"
        self.logger.info(f"Статус фильтрации: {status}")
        
        if iteration_info.get('filter_active', False):
            self.logger.info(f"Удалено генов: {iteration_info['removed_genes']}")
        else:
            self.logger.info("Удалено генов: 0 (фильтрация не выполнялась)")
    ...
    def log_go_info(self, gene, has_anno):
        """Логирование информации о GO аннотациях"""
        status = "НАЙДЕНЫ" if has_anno else "ОТСУТСТВУЮТ"
        self.logger.debug(f"Ген {gene}: аннотации GO {status}")
...
class GOClusterAnalyzer:
    ...
    def prepare_combined_features(self, gene_names, expr_data):
        ...
        expr_features = MinMaxScaler().fit_transform(expr_data.T) ### БОЛЬШОЙ ВОПРОС НАДО ЛИ
...
def main():
    ...
    output_dir = rf'C:\Users\Jacobs\Desktop\go_clustering\go_Relize_Program_clastering_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    
    # Параметры кластеризации
    n_start = 600               # Начальное число кластеров
    n_min = 1000                # Конечное число кластеров
    n_deductible = 100          # Шаг изменения кластеров
    kmeans_trials = 3           # Количество попыток кластеризации (СИЛЬНО ЗАМЕДЛЯЕТ)
    ...
    logger.logger.info("Инициализация программы...")
    logger.logger.info(f"Входной файл: {input_file}")
    logger.logger.info(f"Выходная директория: {output_dir}")
    ...
    while n_current <= n_min and len(current_genes) > n_current:
        ...
        if filter_iterations > 0 and filter_count < filter_iterations:
            # Если фильтрация активна на этой итерации
            iteration_info = {
                'number': iteration,
            ...
            # Если фильтрация неактивна (завершена или отключена)
            iteration_info = {
                'number': iteration,
                'n_clusters': n_current,
                'inertia': best_inertia,
                'removed_genes': 0,  # Явно указываем 0 удалений
                'remaining_genes': len(current_genes),
                'filter_active': False
            }
    ...
    if GO_AVAILABLE and go_analyzer.go_dag:
        ...
        # Сортируем кластеры по размеру (по убыванию)
        sorted_clusters = sorted(clusters.items(), key=lambda x: len(x[1]), reverse=True)
        
        # Анализируем только топ-10 кластеров по размеру + 5 случайных средних/маленьких
        top_clusters = sorted_clusters[:10]
        if len(sorted_clusters) > 15:
            # Добавляем 5 случайных кластеров из оставшихся (кроме самого маленького)
            other_clusters = sorted_clusters[10:-1]
            top_clusters.extend(random.sample(other_clusters, min(5, len(other_clusters))))
        ...
        for cluster_id, genes in top_clusters:
            ...
            for aspect in ['biological_process', 'molecular_function', 'cellular_component']:
                ...
                if aspect_terms:
                    # Берем топ-3 термина для каждого аспекта
                    top_terms = aspect_terms[:3]
                    aspect_str = f"  {aspect}:"
                    logger.logger.info(aspect_str)
                    
                    for i, (term, count) in enumerate(top_terms, 1):
                        term_name = go_analyzer.go_dag[term].name
                        term_str = f"    {i}. {term_name} ({count} генов, {term})"
                        logger.logger.info(term_str)
                else:
                    no_terms_str = f"  {aspect}: нет значимых терминов"
                    logger.logger.info(no_terms_str)
        
        # Добавляем общую статистику
        total_clusters = len(clusters)
        total_genes = sum(len(g) for c, g in clusters.items() if c != 'X')
        avg_genes = total_genes / total_clusters if total_clusters > 0 else 0
        
        stats_str = (
            f"\nОБЩАЯ СТАТИСТИКА КЛАСТЕРОВ:\n"
            f"Всего кластеров: {total_clusters}\n"
            f"Всего генов в кластерах: {total_genes}\n"
            f"Средний размер кластера: {avg_genes:.1f} генов\n"
            f"Проанализировано кластеров: {len(top_clusters)}"
        )
        logger.logger.info(stats_str)
